...
export function Sidebar() {
  ...
  const { data: plugins } = useQuery<PluginInfo[]>({
    queryKey: ['plugins'],
    queryFn: async () => {
      const result = await graphqlQuery<{ plugins: PluginInfo[] }>(queries.listPlugins);
        ...
        {plugins?.map((plugin) => {
          ...
          return (
            ...
            >
              ...
              <IconComponent className={cn('h-4 w-4 ml-1', isActive && 'text-blue-600 dark:text-blue-400')} />
              {plugin.name}
            </Link>
          );
        })}
