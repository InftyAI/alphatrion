...
// Helper function to sync files to pod
async function syncFilesToPod(podName: string, files: Record<string, string>) {
    const response = await fetch(`/api/ai-studio/terminal/${podName}/sync`, {
        method: 'POST',
...
export default function TerminalPanel({
    ...
    useEffect(() => {
        ...
        // Build WebSocket URL for AI Studio
        const apiBase =
        ...
        const wsBase = apiBase
            ...
            .replace(/^http:\/\//, "ws://");
        const wsUrl = wsBase
            ? `${wsBase}/api/ai-studio/terminal/${encodeURIComponent(podName)}`
            : `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${window.location.host}/api/ai-studio/terminal/${encodeURIComponent(podName)}`;
