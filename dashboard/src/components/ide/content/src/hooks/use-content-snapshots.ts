...
export function useContentSnapshotsSummary(trialId: string | null, options?: { refetchInterval?: number | false }) {
    const { data, isLoading, error } = useQuery({
        queryKey: ["contentSnapshotsSummary", trialId],
        queryFn: () => fetchContentSnapshotsSummary({ trialId: trialId! }),
        enabled: !!trialId,
...
export function useContentLineage() {
    ...
    const loadLineage = useCallback(async (trialId: string, contentUid: string) => {
        setIsLoading(true);
        setError(null);
        try {
            const data = await queryClient.fetchQuery({
                queryKey: ["contentLineage", trialId, contentUid],
                queryFn: () => fetchContentLineage({ trialId, contentUid }),
            });
            setLineage(data);
        } catch (err) {
            setError(err as Error);
