import { GitCompare, HelpCircle, Maximize2, Minimize2, RotateCcw, ChevronDown } from "lucide-react";
...
import MetricsTree from "../../components/ide/charts/metrics-tree";
import type { ContentSnapshotSummary, Project, Experiment } from "../../types";
import { extractFitnessKeys, getFitnessValue } from "../../utils/fitness";
// import { ChatInline } from "../../components/ide/chat/ChatInline"; // TODO: Re-enable after fixing module resolution
import ContentComparisonPanel from "../../components/ide/content/content-comparison-panel";
import ContentEvolutionChart, { type ChartViewMode } from "../../components/ide/content/content-evolution-chart";
import LineageHistory from "../../components/ide/content/lineage-history";
import LineageTree from "../../components/ide/content/lineage-tree";
import { FileTree, FileViewer } from "../../components/ide/repo";
import { Button } from "../../components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "../../components/ui/card";
import { EvaluationResult } from "../../components/ui/json-value";
import {
    Select,
    SelectContent,
    SelectItem,
...
const DEBUG_ABLATION = {
    ...
    chatPanel: false,      // Chat panel at bottom of code viewer (temporarily disabled)
...
export default function CloudIDEFull() {
    // Get team context
    const { selectedTeamId } = useTeamContext();

    // URL search params for selecting points
    ...
    // Auto-refresh: poll every 60s while experiment is ongoing
    const [pollInterval, setPollInterval] = useState<number | false>(false);
    const [selectedMetricPaths, setSelectedMetricPaths] = useState<Set<string>>(new Set());

    // Use experiment directly instead of trial
    const { data: experiment, isLoading, error } = useExperiment(selectedExperimentId);

    // Simplified selection context (no trial concepts)
    const [selectedPointName, setSelectedPointName] = useState<string | null>(null);
    const [chartPanelMode, setChartPanelMode] = useState<"evolution" | "metrics">("evolution");
    ...
    }, [isOngoing, experiment]);

    // Load content snapshots summary (lightweight, no content_text)
    const { snapshots, isLoading: snapshotsLoading } = useContentSnapshotsSummary(selectedExperimentId ?? null, { refetchInterval: pollInterval });

    // Prefetch metric keys so they're cached when user switches to metrics view (commented out - need to implement)
    // useMetricKeys(experimentId ?? null, { refetchInterval: pollInterval });
    ...
    const { tree: repoTree } = useRepoFileTree(selectedExperimentId ?? null);
    const { content: repoFileContent, isLoading: repoFileLoading, loadFile: loadRepoFile } = useRepoFileContent(selectedExperimentId ?? null);
    ...
    useEffect(() => {
        const fetchComparisonContent = async () => {
            if (comparisonSnapshot1) {
                const { fetchContentSnapshot } = await import("../../services/graphql");
    ...
    // Removed trial-related useEffect blocks - we're using experiment directly
    ...
    return (
        <div className="flex flex-col" style={{ height: '100vh' }}>
            {/* Header with Project and Experiment Selectors */}
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Select
                        ...
                        onValueChange={(value) => {
                            ...
                            setSelectedExperimentId(null); // Reset experiment when project changes
                        }}
            ...
            {!selectedExperimentId ? (
                        ...
                        <CardContent className="pt-6 text-center">
                            <p className="text-muted-foreground">
                                {!selectedProjectId
                                    ...
                                    : "Please select an experiment to view the IDE."}
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="flex-1 flex items-center justify-center p-6">
                    <Card className="border-destructive bg-destructive/10 w-[400px]">
            ...
            {chartPanelMode === "metrics" ? (
                <div className="flex-1 min-h-0">
                    <MetricsTree experimentId={selectedExperimentId!} isOngoing={isOngoing} experimentCreatedAt={experiment.createdAt} selectedPaths={selectedMetricPaths} onSelectedPathsChange={setSelectedMetricPaths} />
                </div>
            ) : (
                <>
                    ...
                    <Card ref={horizontalContainerRef} className="flex flex-1 min-h-0 overflow-hidden rounded-none border-0">
                        ...
                        >
                            ...
                            {selectedExperimentId && DEBUG_ABLATION.chatPanel && (
                                <ChatInline
