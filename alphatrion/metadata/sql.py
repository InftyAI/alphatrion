...
class SQLStore(MetaStore):
    ...
    def create_model(self, name: str, version: str = "latest", description: str | None = None, meta: dict | None = None, labels: dict | None = None):
        session = self._session()
        new_model = Model(
            name=name,
            version=version,
    ...
    def update_model(self, model_id: int, **kwargs):
        session = self._session()
        model = session.query(Model).filter(Model.id == model_id, Model.is_del == 0).first()
        if model:
            for key, value in kwargs.items():
    ...
    def get_model(self, model_id: int) -> Model | None:
        ...
        model = (
            session.query(Model)
            .filter(Model.id == model_id, Model.is_del == 0)
            .first()
        )
        session.close()
        return model
